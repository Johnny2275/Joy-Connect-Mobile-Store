...
const products = {
  ...
  gaming: [
    ...
    {sku:'GAM-002', name:'Wired USB Gamepad (PC/Console-style)', price:9800, was:null, icon:'🕹️', tag:null, tagLabel:'', rating:'4.5', sold:38, brand:'Infinix'},
  ]
};
...
document.getElementById('search-logo-img').src = document.querySelector('.hero-logo img').src;
document.getElementById('footer-logo-img').src = document.querySelector('.hero-logo img').src;
...
const allProducts = [...products.phone, ...products.laptop, ...products.gaming];
...
function findProduct(sku){
  return [...products.phone, ...products.laptop, ...products.gaming].find(p => p.sku === sku);
}
...
function openModal(sku){
  ...
  document.getElementById('m-stars').innerHTML = `✅ In Stock`;
  updateBreakdown();
...
document.getElementById('pay-btn').addEventListener('click', () => {
  ...
  // --- INTEGRATION POINT ---
  // Replace this block with a real Paystack Inline call in your React app, e.g.:
  //
  // const handler = PaystackPop.setup({
  //   key: 'pk_live_xxx',
  //   email: `${phone}@placeholder.plugpoint.ng`,
